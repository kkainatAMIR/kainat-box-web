...
const categories = [
  { name: 'Custom Pizza Boxes', slug: 'pizza-boxes', image: '/images/pizza-front.jpg', kicker: 'Food-safe & heat smart', blurb: 'Sample photos of custom pizza boxes we manufacture — grease-resistant corrugated boxes engineered for a crisp delivery, made to your brand.' },
  { name: 'Custom Shoe Boxes', slug: 'shoe-boxes', image: '/images/shoe-front.jpg', kicker: 'Retail-ready rigidity', blurb: 'Sample photos of custom shoe boxes we manufacture — premium rigid and folding formats that turn unboxing into a brand moment.' },
  { name: 'Custom Bakery Boxes', slug: 'bakery-boxes', image: '/images/bakery-front.jpg', kicker: 'Freshness, beautifully framed', blurb: 'Sample photos of custom bakery boxes we manufacture — food-grade cake, pastry and window boxes with custom-fit inserts.' },
  { name: 'Custom Lollipop Boxes', slug: 'lollipop-boxes', image: '/images/lollipop-front.jpg', kicker: 'Display-ready sweetness', blurb: 'Sample photos of custom lollipop boxes we manufacture — food-safe individual, window and counter-display packs made for confectionery.' },
  { name: 'Custom Cartons', slug: 'cartons', image: '/images/carton-front.jpg', kicker: 'Built for the journey', blurb: 'Sample photos of custom cartons we manufacture — tough corrugated shipping cartons for storage, logistics and ecommerce.' },
  { name: 'Custom Packaging', slug: 'custom-packaging', image: '/images/custom-front.jpg', kicker: 'Made without limits', blurb: 'Sample photos of custom packaging we manufacture — purpose-built structures, finishes and inserts shaped around your product.' },
];

const boxViews = {
  pizza: [
    { src: '/images/pizza-front.jpg', view: 'Front' },
    { src: '/images/pizza-side.jpg', view: 'Side' },
    { src: '/images/pizza-inside.jpg', view: 'Inside' },
  ],
  shoe: [
    { src: '/images/shoe-front.jpg', view: 'Front' },
    { src: '/images/shoe-top.jpg', view: 'Top' },
    { src: '/images/shoe-inside.jpg', view: 'Inside' },
  ],
  bakery: [
    { src: '/images/bakery-front.jpg', view: 'Front' },
    { src: '/images/bakery-side.jpg', view: 'Side' },
    { src: '/images/bakery-inside.jpg', view: 'Inside' },
  ],
  lollipop: [
    { src: '/images/lollipop-front.jpg', view: 'Front' },
    { src: '/images/lollipop-side.jpg', view: 'Side' },
    { src: '/images/lollipop-inside.jpg', view: 'Inside' },
  ],
  carton: [
    { src: '/images/carton-front.jpg', view: 'Front' },
    { src: '/images/carton-side.jpg', view: 'Side' },
    { src: '/images/carton-inside.jpg', view: 'Inside' },
  ],
  custom: [
    { src: '/images/custom-front.jpg', view: 'Front' },
    { src: '/images/custom-top.jpg', view: 'Top' },
    { src: '/images/custom-inside.jpg', view: 'Inside' },
  ],
  whiteCarton: [
    { src: '/images/white-carton-front.jpg', view: 'Front' },
    { src: '/images/white-carton-top.jpg', view: 'Top' },
    { src: '/images/white-carton-inside.jpg', view: 'Inside' },
  ],
};

const categoryAliases = {
  'Pizza Boxes': 'Custom Pizza Boxes',
  'Shoe Boxes': 'Custom Shoe Boxes',
  'Bakery Boxes': 'Custom Bakery Boxes',
  'Lollipop Boxes': 'Custom Lollipop Boxes',
  'Cartons': 'Custom Cartons',
};

function galleryFor(product) {
  return (product.images?.length ? product.images : [{ src: product.image, view: 'Front' }]).slice(0, 3);
}

const products = [
  { id: 'kraft-pizza-12', name: 'Custom Kraft Pizza Box — 12″', category: 'Custom Pizza Boxes', material: 'Corrugated Kraft', size: '12 × 12 × 1.8 in', price: 165, moq: 100, image: boxViews.pizza[0].src, images: boxViews.pizza, desc: 'Sample of our custom 12″ pizza boxes. Food-safe, grease-resistant E-flute with steam vents — we make these to your size, print and brand.', featured: true, specs: ['Food-grade kraft board', 'Heat-retaining E-flute', 'Single-color print included', '100% recyclable'] },
  { id: 'premium-pizza-14', name: 'Custom Premium Pizza Box — 14″', category: 'Custom Pizza Boxes', material: 'White Corrugated', size: '14 × 14 × 1.8 in', price: 195, moq: 100, image: boxViews.pizza[0].src, images: boxViews.pizza, desc: 'Sample of our custom 14″ pizza boxes. Bright white print surface for restaurant branding — made to your artwork, size and quantity.', specs: ['Food-safe white liner', 'CMYK-ready surface', 'Easy-lock corners', 'Recyclable'] },
  { id: 'slide-shoe-box', name: 'Custom Slide Drawer Shoe Box', category: 'Custom Shoe Boxes', material: 'Rigid Board', size: '13 × 8 × 5 in', price: 420, moq: 50, image: boxViews.shoe[0].src, images: boxViews.shoe, desc: 'Sample of our custom drawer shoe boxes. A premium pull-out format with ribbon tab — we make these to your brand, size and finish.', featured: true, specs: ['1200gsm rigid board', 'Textured paper wrap', 'Cotton pull tab', 'Foil or emboss available'] },
  { id: 'classic-shoe-box', name: 'Custom Classic Two-Piece Shoe Box', category: 'Custom Shoe Boxes', material: 'Kraft Board', size: '13 × 8 × 5 in', price: 275, moq: 100, image: boxViews.shoe[0].src, images: boxViews.shoe, desc: 'Sample of our custom lid-and-base shoe boxes. Durable retail construction — printed and sized for your footwear brand.', specs: ['600gsm kraft board', 'Scuff resistant', 'Pantone printing', 'Custom insert available'] },
  { id: 'window-pastry-box', name: 'Custom Window Pastry Box', category: 'Custom Bakery Boxes', material: 'Food-grade Kraft', size: '10 × 8 × 3 in', price: 145, moq: 100, image: boxViews.bakery[0].src, images: boxViews.bakery, desc: 'Sample of our custom bakery window boxes. A clear plant-based window puts your bakes on show — made to your size and print.', featured: true, specs: ['Food contact certified', 'PLA window option', 'Auto-lock base', 'Flat-packed delivery'] },
  { id: 'cake-box-12', name: 'Custom Tall Cake Box — 12″', category: 'Custom Bakery Boxes', material: 'SBS Paperboard', size: '12 × 12 × 10 in', price: 225, moq: 50, image: boxViews.bakery[0].src, images: boxViews.bakery, desc: 'Sample of our custom tall cake boxes. Sturdy format with a removable front panel — we make these to your bakery’s size and brand.', specs: ['350gsm food board', 'Reinforced base', 'Optional carry handle', 'Grease resistant'] },
  { id: 'lollipop-display-box', name: 'Custom Lollipop Counter Display Box', category: 'Custom Lollipop Boxes', material: 'Food-grade Kraft', size: '9 × 7 × 6 in', price: 175, moq: 100, image: boxViews.lollipop[0].src, images: boxViews.lollipop, desc: 'Sample of our custom lollipop display boxes. A retail counter pack that keeps sweets upright — printed and fitted for your brand.', featured: true, specs: ['Food-safe kraft board', 'Custom display insert', 'Full-color print option', 'Flat-packed delivery'] },
  { id: 'single-lollipop-window-box', name: 'Custom Single Lollipop Window Box', category: 'Custom Lollipop Boxes', material: 'Food-grade Kraft', size: '3 × 2 × 8 in', price: 95, moq: 100, image: boxViews.lollipop[0].src, images: boxViews.lollipop, desc: 'Sample of our custom single lollipop boxes. Slim window presentation for artisan sweets — we make these to your size and artwork.', specs: ['Food-contact safe board', 'Clear PLA window option', 'Custom fit insert', 'Ribbon and foil available'] },
  { id: 'mailer-carton', name: 'Custom Self-Lock Mailer Carton', category: 'Custom Cartons', material: 'E-flute Corrugated', size: '12 × 9 × 4 in', price: 185, moq: 100, image: boxViews.carton[0].src, images: boxViews.carton, desc: 'Sample of our custom ecommerce mailer cartons. Tape-free presentation with real protection — made to your dimensions and print.', featured: true, specs: ['3-layer E-flute', 'Crash-lock front', 'Inside/outside print', 'Tear-strip available'] },
  { id: 'shipping-carton', name: 'Custom Heavy-Duty Shipping Carton', category: 'Custom Cartons', material: 'B-flute Corrugated', size: '18 × 14 × 12 in', price: 260, moq: 50, image: boxViews.carton[0].src, images: boxViews.carton, desc: 'Sample of our custom shipping cartons. Stackable transport boxes for demanding supply chains — we make these to your size and brand.', specs: ['5-ply option', 'Burst-tested board', 'Flexographic print', 'Custom dimensions'] },
  { id: 'white-corrugated-carton', name: 'Custom White Corrugated Carton', category: 'Custom Cartons', material: 'White Corrugated', size: '12 × 12 × 10 in', price: 230, moq: 50, image: boxViews.whiteCarton[0].src, images: boxViews.whiteCarton, desc: 'Sample of our custom white shipping cartons. A crisp surface for retail, gifting and ecommerce — printed or plain, made to your size.', specs: ['Bright white outer liner', 'Protective corrugated board', 'Plain or minimal one-color print', 'Custom dimensions available'] },
  { id: 'hex-gift-box', name: 'Custom Hexagonal Gift Box', category: 'Custom Packaging', material: 'Rigid Board', size: 'Custom', price: 520, moq: 50, image: boxViews.custom[0].src, images: boxViews.custom, desc: 'Sample of our custom hexagonal gift boxes. An architectural rigid format for beauty and specialty retail — built around your product.', specs: ['Bespoke structure', 'Magnetic closure option', 'Premium foil finishes', 'Custom foam insert'] },
  { id: 'paper-tube', name: 'Custom Premium Paper Tube', category: 'Custom Packaging', material: 'Recycled Paper', size: 'Custom', price: 310, moq: 100, image: boxViews.custom[0].src, images: boxViews.custom, desc: 'Sample of our custom paper tubes. Circular packaging for cosmetics, tea, candles and gifting — we make these to your size and brand.', specs: ['Recycled paper core', 'Food-safe liner option', 'Embossed wrap', 'Metal-free construction'] },
];
...
const pageMeta = {
  ...
  '/products': ['Shop Custom Boxes & Cartons', 'Browse sample photos of custom pizza, shoe, bakery and lollipop boxes, cartons, and fully custom packaging we manufacture to your brand.'],
  ...
  return <section className="motion-story" ref={root} aria-label="Scroll-controlled packaging showcase">
    <div className="motion-stage" ref={stage} data-scene="intro">
      ...
      <div className="motion-scene motion-shoe-scene">
        ...
        <img className="motion-shoebox" src="/images/motion-shoebox.jpg" alt="Open KAB custom shoe box sample" />
      ...
      <div className="motion-scene motion-pizza-scene">
        ...
        <img className="motion-pizzabox" src="/images/motion-pizzabox.jpg" alt="Open KAB custom pizza box sample" />
      ...
      <div className="motion-finale">
        ...
        <div className="finale-stack"><img src="/images/bakery-front.jpg" alt="KAB custom bakery box sample"/><img src="/images/carton-front.jpg" alt="KAB custom shipping carton samples"/><img src="/images/custom-front.jpg" alt="KAB custom packaging samples"/></div>
      </div>
  ...
  return <section className="hero">
    ...
    <div className="container hero-grid">
      ...
      <div className="hero-visual">
        <div className="image-stage"><img src="/images/packforge-hero.jpg" alt="Sample collection of KAB custom pizza, shoe, bakery and shipping boxes" /></div>
...
function CategoryCard({ cat, index }) {
  return <Reveal className={`category-card card-${index}`}>
    <Link to={`/products?category=${encodeURIComponent(cat.name)}`}>
      <div className="category-image"><img src={cat.image} alt={`${cat.name} sample photos`} loading="lazy"/><span className="category-index">0{index + 1}</span><span className="sample-pill">Sample · custom</span></div>
...
function WhyUs() {
  ...
  return <section className="section why-section"><div className="container"><SectionHeading eyebrow="WHY KAINAT" title={<>Details matter.<br/><em>So we mind every one.</em></>} text="We blend practical engineering with thoughtful design to create packaging that performs at every touchpoint."/><div className="why-layout"><div className="why-image"><img src="/images/custom-front.jpg" alt="Sample photos of custom KAB packaging we manufacture" loading="lazy"/><div className="why-stamp"><Award/><strong>15 YEARS</strong><span>PACKAGING CRAFT</span></div></div><div className="why-grid">{items.map(([Icon,t,p])=><Reveal className="why-card" key={t}><Icon/><div><h3>{t}</h3><p>{p}</p></div></Reveal>)}</div></div></div></section>
}

function ProductCard({ product, addToCart }) {
  return <article className="product-card">
    <Link to={`/products/${product.id}`} className="product-image"><img src={product.image} alt={`${product.name} sample photo`} loading="lazy"/>{product.featured && <span className="pill">Bestseller</span>}<span className="sample-pill">Sample · custom</span><span className="quick-view">View details <ArrowRight size={15}/></span></Link>
...
function FeaturedProducts({ addToCart }) {
  return <section className="section featured-products"><div className="container"><SectionHeading eyebrow="SAMPLE CUSTOM WORK" title={<>Popular custom boxes,<br/><em>made to your brand.</em></>} text="Photos are samples. We manufacture each format to your size, print and artwork." action={<Link to="/products" className="text-link">Shop all products <ArrowRight/></Link>}/><div className="product-grid">{products.filter(p=>p.featured).slice(0,4).map(p=><ProductCard key={p.id} product={p} addToCart={addToCart}/>)}</div></div></section>
}
...
function ProductsPage({ addToCart }) {
  ...
  const [category, setCategory] = useState(categoryAliases[params.get('category')] || params.get('category') || 'All products');
  ...
  return <><PageHero eyebrow="THE PRODUCT FLOOR" title={<>Find your<br/><em>perfect format.</em></>} text="These are sample photos of custom boxes we manufacture. Every format can be made to your size, print and brand."><div className="hero-mini-stat"><strong>13+</strong><span>sample formats<br/>∞ custom possibilities</span></div></PageHero>
...
function ProductDetail({ id, addToCart }) {
  ...
  const gallery=p?galleryFor(p):[];
  const [qty,setQty]=useState(p?.moq||1);
  const [finish,setFinish]=useState('Natural kraft');
  const [active,setActive]=useState(0);
  useEffect(()=>{setActive(0);setQty(p?.moq||1);},[id]);
  if(!p) return <NotFound/>;
  const shot=gallery[Math.min(active,gallery.length-1)]||{src:p.image,view:'Front'};
  return <><div className="breadcrumb container"><Link to="/">Home</Link><span>/</span><Link to="/products">Products</Link><span>/</span><b>{p.name}</b></div><section className="product-detail container"><div className="detail-gallery"><div className="detail-main"><img src={shot.src} alt={`${p.name} ${shot.view.toLowerCase()} sample view`}/><span className="detail-tag">{shot.view} view</span><span className="sample-flag">Sample photos · we make this box custom</span></div><div className="detail-thumbs">{gallery.map((img,i)=><button key={img.view} type="button" className={active===i?'selected':''} onClick={()=>setActive(i)}><img src={img.src} alt={`${p.name} ${img.view} sample`}/><small>{img.view}</small></button>)}</div></div><div className="detail-copy"><span className="eyebrow">{p.category}</span><h1>{p.name}</h1><div className="detail-rating"><span>★★★★★</span><b>4.9</b><a href="#reviews">24 verified reviews</a></div><p className="detail-desc">{p.desc} Photos are samples of custom work. We manufacture this format to your size, print and brand with dependable board and precise cutting.</p><div className="price-line"><strong>PKR {p.price.toLocaleString()}</strong><span>/ unit from {p.moq} units</span></div><div className="option-block"><div><label>Finish</label><small>Selected: {finish}</small></div><div className="finish-options">{['Natural kraft','Forest green','Brilliant white'].map((f,i)=><button key={f} onClick={()=>setFinish(f)} className={finish===f?'selected':''}><i className={`finish-${i}`}></i>{f}</button>)}</div></div><div className="option-block"><div><label>Size</label><a href="mailto:hello@kainatboxmakers.com">Need a custom size?</a></div><select defaultValue={p.size}><option>{p.size}</option><option>Custom dimensions</option></select></div><div className="detail-buy"><div className="qty"><button onClick={()=>setQty(Math.max(p.moq,qty-p.moq))}><Minus/></button><span>{qty}</span><button onClick={()=>setQty(qty+p.moq)}><Plus/></button></div><button className="btn btn-dark" onClick={()=>addToCart(p,qty)}>Add {qty} to cart <ShoppingBag/></button></div><Link to="/quote" className="custom-quote-link"><FileUp/> Custom print, size or structure? <b>Request a tailored quote</b><ArrowRight/></Link><div className="detail-trust"><span><CircleCheck/> Production proof included</span><span><Clock3/> Typical lead time 10–15 days</span><span><Truck/> Nationwide delivery</span></div></div></section><section className="detail-spec-section"><div className="container"><div><span className="eyebrow">PRODUCT SPECIFICATION</span><h2>Built to perform.<br/><em>Finished to impress.</em></h2></div><div className="spec-table"><div><span>Material</span><strong>{p.material}</strong></div><div><span>Standard size</span><strong>{p.size}</strong></div><div><span>Minimum order</span><strong>{p.moq} units</strong></div><div><span>Print options</span><strong>Flexo, CMYK, Pantone</strong></div>{p.specs.map((s,i)=><div key={s}><span>Feature {String(i+1).padStart(2,'0')}</span><strong>{s}</strong></div>)}</div></div></section><section className="section related"><div className="container"><SectionHeading eyebrow="YOU MAY ALSO LIKE" title="More ways to pack it."/><div className="product-grid">{products.filter(x=>x.id!==p.id).slice(0,4).map(x=><ProductCard key={x.id} product={x} addToCart={addToCart}/>)}</div></div></section></>
}
...
function QuotePage({ showToast }) {
  ...
  const [form,setForm]=useState({boxType:'Custom Pizza Boxes',quantity:'500 – 1,000',material:'Recommend for me'});
...
function App(){
 ...
 else if(routePath.startsWith('/products/'))page=<ProductDetail key={routePath.split('/')[2]} id={routePath.split('/')[2]} addToCart={addToCart}/>;
